...
class WebResponse {
    ...
    /**
     * Returns the specified cookie or `null` if not found.
     *
     * @param string $name
     * @param bool $ignoreCharacterCase
     * @return WebResponseCookie|null
     */
    public function getCookie($name, $ignoreCharacterCase = true) {
        $name = $ignoreCharacterCase ? strtolower($name) : $name;

        foreach ($this->getCookies() as $cookie) {
            $cookieName = $ignoreCharacterCase ? strtolower($cookie->getName()) : $cookie->getName();
    ...
    /**
     * Returns the HTTP message, such as `200 OK` or `404 Not Found`, as the remote server sent it. In some cases, the
     * remote server will not include the text after the status code.
     *
     * @return string
     */
